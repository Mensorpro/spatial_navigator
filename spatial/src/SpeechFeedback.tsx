  ...
  const speakText = (text: string, isUrgent = false, isInformational = false) => {
    if ('speechSynthesis' in window) {
      ...
      if (isUrgent) {
        ...
        speech.rate = 1.0; // Normal speed for summaries and path guidance
  ...
  return (
    <div className="absolute bottom-4 left-4 right-4 bg-black bg-opacity-70 text-white p-3 rounded-lg z-10">
      <div className="flex flex-col gap-2">
        ...
        {navigationData.paths.length > 0 && (
          <div className="text-sm mt-1 border-t border-gray-600 pt-1">
            <div className="font-semibold">Available Paths:</div>
            <ul className="list-disc list-inside">
              {navigationData.paths.map((path, idx) => (
                <li key={idx}>{path}</li>
              ))}
            </ul>
          </div>
        ...
        {navigationData.warnings.length > 0 && (
          <div className="text-sm mt-1 border-t border-gray-600 pt-1 text-red-300">
            <div className="font-semibold">⚠️ Warnings:</div>
            <ul className="list-disc list-inside">
              {navigationData.warnings.map((warning, idx) => (
